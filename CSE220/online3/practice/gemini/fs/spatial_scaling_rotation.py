...
if __name__ == "__main__":
    ...
    z_sclaed_rotated=2*np.exp(1j*np.pi/4)*z
    fs_sclaed_rotated=FourierEpicycles(t, z_sclaed_rotated, n_harmonics=N_HARMONICS )
    fs_sclaed_rotated.calculate_all_coefficients()

    
    n_values = np.array(list(fs_orig.coeffs.keys()))

    c_orig = np.array(list(fs_orig.coeffs.values()))
    c_scaled_rotated=np.array(list(fs_sclaed_rotated.coeffs.values()))
    c_theory = c_orig*2*np.exp(1j*np.pi/4)
