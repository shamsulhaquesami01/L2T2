...
if __name__ == "__main__":
    t1,z1=load_svg_path("star.svg",1000)
    t2,z2=load_svg_path("circle.svg",1000)
    fs_z1=FourierEpicycles(t1,z1,100)
    fs_z1.calculate_all_coefficients();
    cn_z1=np.array(list(fs_z1.coeffs.values()))
    fs_z2=FourierEpicycles(t1,z2,100)
    fs_z2.calculate_all_coefficients();
    cn_z2=np.array(list(fs_z2.coeffs.values()))
    h=0.5*z1+0.5*z2
    fs_h=FourierEpicycles(t1,h,100)
    fs_h.calculate_all_coefficients();
    cn_h=np.array(list(fs_h.coeffs.values()))
    n_values=np.array(list(fs_z1.coeffs.keys()))

    cn_h_theory=0.5*cn_z1+0.5*cn_z2
    mask = np.abs(cn_h) > 1e-6
    mse_mag,mse_phase=calculate_mse(cn_h,cn_h_theory,mask=mask)

    # magnitude overlay
    fig, axes = plt.subplots(2, 1, figsize=(9, 7))
    ax = axes[0]
    ax.stem(n_values, np.abs(cn_h), linefmt='C0-', markerfmt='C0o', basefmt=' ', label='|cn| (original)')
    ax.stem(n_values, np.abs(cn_h_theory), linefmt='C1--', markerfmt='C1x', basefmt=' ', label='|dn| (shifted, measured)')
    ax.set_xlabel('n')
    ...
    ax.stem(n_masked, np.angle(cn_h)[mask], linefmt='C1--', markerfmt='C1x', basefmt=' ', label='∠dn (measured)')
    ax.stem(n_masked, np.angle(cn_h_theory)[mask], linefmt='C2:', markerfmt='C2^', basefmt=' ', label='∠dn (theory)')
    ax.set_xlabel('n')
