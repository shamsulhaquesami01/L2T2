...
class LTISystem:
    ...
    def output(self, input_signal):
        ...
        return y
    def output_super(self, input_sig: SuperSignal):
        result = None
        for coefficient, signal in input_sig.components:
            y = self.output(signal).multiply(coefficient)
            result = y if result is None else result.add(y)
...
if __name__ == "__main__":
    ...
    final = system.output_super(ss)
    final.plot("Superposition output")
    plt.show()
